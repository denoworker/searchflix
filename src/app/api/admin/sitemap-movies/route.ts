...
// GET /api/admin/sitemap-movies - Get all sitemap movies or movies from specific sitemap
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error fetching sitemap movies:', error);
    return NextResponse.json(
...
// DELETE /api/admin/sitemap-movies - Delete a sitemap movie
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('Error deleting sitemap movie:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
